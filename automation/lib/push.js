...
export async function sendBrief({ hook, slug, title, description, image, url }) {
  ...
  try {
    ...
    const payload = JSON.stringify({ title: hook || title || '오늘의 브리핑', body: description || title || '', image: image || undefined, url, tag: 'crama-brief' });
    for (const s of rows) {
      try {
    ...
    // 발송 성과 기록(발송 수). 클릭은 UTM→GA4 로 집계.
    try { await client.query('insert into public.push_sends (slug, title, sent, dead) values ($1,$2,$3,$4)', [slug || null, hook || title || null, sent, dead]); } catch (e) {}
  } finally {
    await client.end();
  }
